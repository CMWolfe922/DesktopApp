...
def generateCode():
	...
	messagebox.showinfo("Blaze Inventory QR Code Generator","QR Code is saved successfully!")

class BlazeInventoryManager(ctk.CTk):

	APP_NAME = "Blaze Inventory Manager"
	WIDTH = 1000
	HEIGHT = 850
...
class QRCodeFrame(ctk.Frame):

	FRAME_NAME = "Blaze Inventory Manager"
	WIDTH = 1000
	HEIGHT = 850
	...
	def __init__(self, *args, **kwargs):
		...
		# Build the Heading Frame
		self.headingFrame = Frame(master=self, bg="azure", bd=5)
		self.headingFrame.place(relx=0.15, rely=0.05, relwidth=0.7, relheight=0.7)
		self.headingLabel = Label(self.headingFrame, text="Inventory Manager: QR Code Generator", bg="azure", font=('Verdana', 20, 'bold'))
		self.headingLabel.place(relx=0, rely=0, relwidth=1, relheight=1)

		# Build the text entry Frame to get the QR Code

# Creating the window
# wn = Tk()
# wn.title('Blaze Express Courier - Inventory QR Code Generator')
